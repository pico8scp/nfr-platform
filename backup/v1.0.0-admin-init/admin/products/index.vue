        ...
        <table class="d-table">
          <thead>
            <tr>
              ...
              <th style="width:72px">商品图片</th>
              <th style="min-width:200px">商品信息</th>
              <th style="width:80px">来源</th>
              <th style="width:100px">销售国家</th>
              <th style="width:80px">价格</th>
              <th style="width:80px">库存</th>
              <th style="width:90px">7日销量</th>
              <th style="width:90px">7日销售额</th>
              <th style="width:100px">AI分析</th>
              <th style="width:70px">转化率</th>
              <th style="width:60px">评分</th>
              <th style="width:95px">C/B上下架</th>
              <th style="width:120px">操作</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="p in filteredProducts" :key="p.id" style="transition:background 0.15s;height:64px;vertical-align:middle">
              <td>
                <input type="checkbox" class="d-checkbox" :checked="selectedIds.includes(p.id)" @change="toggleSelect(p.id)" />
              </td>
              <td>
                <div style="width:56px;height:56px;border-radius:6px;overflow:hidden;background:var(--d-hover);cursor:pointer" @click="openDetail(p)">
                  <img :src="p.image" style="width:100%;height:100%;object-fit:cover" :alt="p.name" />
                </div>
              </td>
              <td>
                <div style="cursor:pointer" @click="openDetail(p)">
                  <div style="font-weight:600;font-size:13px;color:var(--d-text);margin-bottom:4px;line-height:1.4" class="d-truncate">{{ p.name }}</div>
                  <div class="d-text-xs d-text-2">SKU: {{ p.sku }} | ID: {{ p.id }}</div>
                  <div class="d-flex d-g-4 d-mt-4">
                    <span class="d-badge d-badge-gray">{{ p.category }}</span>
                    <span v-if="p.isNew" class="d-badge d-badge-green">新品</span>
                    <span v-if="p.isHot" class="d-badge d-badge-orange">热销</span>
                    <span v-if="p.aiScore < 60" class="d-badge d-badge-red">AI预警</span>
                  </div>
                </div>
              </td>
              <td>
              ...
              <td style="max-width:120px">
                <span class="d-text-sm d-text-2 d-truncate" :title="p.countries.map(c => countryName(c)).join('; ')">{{ displayCountries(p.countries) }}</span>
              </td>
              <td>
                <div style="font-weight:700;font-size:13px;color:var(--d-text)">{{ p.cPrice }}</div>
                <div class="d-text-xs d-text-2 d-mt-2">B: {{ p.bPrice }}</div>
              </td>
              <td>
                <div :style="{ fontWeight: 600, color: p.stock === 0 ? 'var(--d-red)' : p.stock < 10 ? 'var(--d-orange)' : 'var(--d-text)' }">{{ p.stock }}</div>
                <div v-if="p.stock < 10" class="d-text-xs" style="color:var(--d-red)">不足</div>
              </td>
              <td>
                <div style="font-weight:600;color:var(--d-text)">{{ p.sales7d.toLocaleString() }}</div>
                <div class="d-text-xs" :style="{ color: p.sales7d >= p.sales7dPrev ? 'var(--d-green)' : 'var(--d-red)' }">
                  {{ p.sales7d >= p.sales7dPrev ? '↑' : '↓' }}{{ Math.abs(p.sales7d - p.sales7dPrev).toLocaleString() }}
                </div>
              </td>
              <td>
                <div style="font-weight:600;color:var(--d-text)">¥{{ (p.sales7d * parseFloat(p.cPrice.replace(/[^0-9.]/g,''))).toLocaleString() }}</div>
                <div class="d-text-xs d-text-2 d-mt-2">C端</div>
              </td>
              <td>
                <div class="d-flex d-align-center d-g-8">
                  <div style="width:36px;height:36px;border-radius:50%;position:relative;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700" :style="{ background: aiColor(p.aiScore), color: '#fff' }">
                    {{ p.aiScore }}
                    <svg style="position:absolute;inset:0" viewBox="0 0 36 36" width="36" height="36">
                      <circle cx="18" cy="18" r="16" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="2.5" />
                      <circle cx="18" cy="18" r="16" fill="none" stroke="#fff" stroke-width="2.5" stroke-dasharray="100.53" :stroke-dashoffset="100.53 - (p.aiScore / 100 * 100.53)" stroke-linecap="round" transform="rotate(-90 18 18)" />
                    </svg>
                  </div>
                  <div>
                    <div style="font-size:11px;color:var(--d-text-2)">利润率</div>
                    <div style="font-size:12px;font-weight:600" :style="{ color: p.margin >= 20 ? 'var(--d-green)' : p.margin >= 10 ? 'var(--d-orange)' : 'var(--d-red)' }">{{ p.margin }}%</div>
                  </div>
                </div>
              </td>
              <td>
                <div style="font-weight:600;color:var(--d-text)">{{ p.convertRate }}%</div>
                <div class="d-text-xs d-text-2">转化</div>
              </td>
              <td>
                <div class="d-flex d-g-2 d-align-center">
                  <span style="color:#FFD700;font-size:12px">★</span>
                  <span style="font-weight:600;font-size:12px;color:var(--d-text)">{{ p.rating }}</span>
                </div>
                <div class="d-text-xs d-text-2">{{ p.reviews }}条</div>
              </td>
              <td>
                <div class="d-flex d-flex-col d-g-4">
                  <button
              ...
              <td>
                <div class="d-flex d-g-4">
                  ...
                  <button class="d-btn d-btn-sm d-btn-outline" style="font-size:11px;padding:3px 8px" @click="copyProduct(p)">复制</button>
                </div>
                <button class="d-btn d-btn-sm d-btn-danger d-mt-4" style="font-size:11px;padding:3px 8px" @click="deleteProduct(p)">删除</button>
              </td>
            </tr>
          </tbody>
        </table>
